...
def read_img(env, path, size=None, data_range=None):
...
if __name__ == '__main__':
    ...
    for pic in pics:
        ...
        rlt_lr = np.around(imresize_np(img_GT, scale)).astype('u1')
        print('down-sample result of {}'.format(pic), ' max=', (rlt_lr - img_LR).max(), ' min=', (rlt_lr - img_LR).min())
        rlt_hr = np.around(imresize_np(img_LR, 1. / scale)).astype('u1')
        print('up-sample result of {}'.format(pic), ' max=', (rlt_hr - img_HR).max(), ' min=', (rlt_hr - img_HR).min())
    print('finish')
